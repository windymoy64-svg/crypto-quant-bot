...
</tr>`; const card=`<article class="mc-card ${dirCls}" data-symbol="${esc(sym)}"><div class="mc-top"><div class="mc-pair"><span class="mc-sym">${esc(sym)}</span><span class="mc-dir ${dirCls}">${dirLabel}</span></div><div class="mc-right"><strong class="${p.pending_order?"":pnlCls}" id="ao-pnl-m-${esc(sym)}">${p.pending_order?"PENDING":`${pnl>=0?"+":""}${moneyFull(pnl)}`}</strong><small>${p.pending_order?"limit":`${pnlPct>=0?"+":""}${pnlPct.toFixed(2)}% pnl`}</small></div></div><div class="mc-metrics"><div class="mc-metric"><em>Entry</em><span>${fmtPrice(entry)}</span></div><div class="mc-metric"><em>Harga Kini</em><span id="ao-price-m-${esc(sym)}">${fmtPrice(current)}</span></div><div class="mc-metric${slWarn}"><em>Stop Loss</em><span>${sl?fmtPrice(sl):"-"}</span>${sl&&distToSl?`<small>${distToSl.toFixed(2)}%</small>`:""}</div><div class="mc-metric${tpWarn}"><em>Take Profit</em><span>${tp1?fmtPrice(tp1):"-"}</span>${tp1&&distToTp?`<small>${distToTp.toFixed(2)}%</small>`:""}</div><div class="mc-metric"><em>Trailing</em><span>${trailing?fmtPrice(trailing):"-"}${trailingActive?" ●":""}</span></div><div class="mc-metric"><em>Modal</em><span>${p.pending_order?"WAIT":moneyFull(modal)}</span><small>${p.pending_order?`exp ${esc(p.expires_at??"-")}`:`${fmt(size)} unit`}</small></div></div></article>`; return {tr,card}; }); n.innerHTML=`<div class="ao-scroll desktop-only"><table class="ao-table"><thead><tr><th>Symbol</th><th>Arah</th><th>Entry</th><th>Harga Kini</th><th>Stop Loss</th><th>Take Profit</th><th>Trailing</th><th>Modal</th><th>Unrealized PnL</th></tr></thead><tbody>${rows.map(r=>r.tr).join("")}</tbody></table></div><div class="mc-list mobile-only">${rows.map(r=>r.card).join("")}</div>`; }
