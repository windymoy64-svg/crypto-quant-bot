...
async function loadAll(){ if(dashboardRefreshRunning) return; dashboardRefreshRunning=true; const start=performance.now(); if(state.loading) setLoading(true); const endpoints=[["market","/api/market"],["portfolio","/api/portfolio"],["multiPortfolio","/api/portfolio/multi"],["paper","/api/paper"],["analytics","/api/analytics"],["liveOrders","/api/live/orders"],["health","/api/health"]]; const payload=clone(state.lastPayload||DEFAULT_PAYLOAD); try{ const results=await Promise.allSettled(endpoints.map(([,p])=>getJson(p))); results.forEach((res,i)=>{ const [key,path]=endpoints[i]; if(res.status==="fulfilled") payload[key]=res.value; else console.warn(`Failed to load ${path}`,res.reason); }); text("latency-badge",`Latency ${Math.round(performance.now()-start)} ms`); render(payload); renderExecutionModeSummary(payload.multiPortfolio); if(typeof window.syncLivePanels==="function") window.syncLivePanels(payload); }finally{ dashboardRefreshRunning=false; setLoading(false); } }
...
function renderAgentEntries(items){
  ...
  const rows = items.map(item => {
    ...
    const action = esc(processing ? "PROCESSING" : (executionFailed ? "ENTRY_REJECTED" : (decision.action || (conflictRejected ? "REJECT" : (res.eligible ? "-" : "SKIP")))));
...
initUi(); loadAll().catch(handleError); connectWs(); loadLLMSettings(); loadExchangeSettings(); loadPortfolioSettings(); loadExecutionSettings(); loadFuturesSettings(); loadAgentPanels().catch(console.warn);
...
setInterval(() => loadAll().catch(handleError), 10000);
