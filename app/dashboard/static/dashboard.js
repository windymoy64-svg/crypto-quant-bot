...
function orderHistory(orders){ const rows=list(orders?.closed_positions).length?[...list(orders.closed_positions),...list(orders?.order_history).filter(o=>o?.close_scope==="partial"||String(o?.bot_role||"").startsWith("take_profit_"))]:list(orders?.order_history).filter(o=>o?.close_scope==="partial"||String(o?.bot_role||"").startsWith("take_profit_")); return rows.map(o=>{ const isPartial=o?.close_scope==="partial"||String(o?.bot_role||"").startsWith("take_profit_"); return {...o,reason:isPartial?(o.close_label??o.reason??o.close_reason):(o.reason??o.close_label??o.close_reason),reason_source:o.reason_source??"exchange_lifecycle"}; }); }
...
function patchOrderHistory(node,rowsHtml){ const tbody=ensureShell(node,OH_SHELL,".order-history-scroll .order-history-table > tbody"); if(!tbody) return; keepScroll([node,node.querySelector(".order-history-scroll")],()=>{ tbody.innerHTML=rowsHtml; tbody.querySelectorAll(".order-row").forEach(row=>{ const reason=row.querySelector(".order-reason span")?.textContent||""; if(!reason.toLowerCase().startsWith("partial close")) return; const dir=row.querySelector(".order-dir"); if(dir) dir.textContent=dir.textContent.replace("CLOSED","PARTIAL"); const badge=row.querySelector(".order-badge"); if(badge){ badge.textContent="PARTIAL"; badge.classList.remove("status-filled"); badge.classList.add("status-partial"); } }); }); }
...
function activeOrderTakeProfits(p){ const raw=p?.take_profits??p?.take_profit??p?.tp_levels??p?.tpPrice??p?.takeProfitPrice??p?.takeProfit??p?.take_profit_price; const values=Array.isArray(raw)?raw:(raw==null?[]:[raw]); return values.map(Number).filter(value=>Number.isFinite(value)&&value>0); }
function renderActiveOrders(pos,pending=[]){ const n=byId("active-orders"); if(!n) return; const all=[...(pos||[]),...(pending||[]).map(o=>({...o,pending_order:true,last_price:o.current_price??o.entry}))]; if(!all.length){ n.innerHTML=emptyState("Tidak ada order aktif","Posisi dan limit order aktif akan muncul di sini."); return; } const rows=all.map(p=>{ const sym=p.symbol??"unknown"; const side=String(p.side||"BUY").toUpperCase(); const isShort=side==="SHORT"||side==="SELL"; const dirLabel=p.pending_order?`${isShort?"SHORT":"LONG"} LIMIT`:(isShort?"SHORT":"LONG"); const dirCls=isShort?"dir-short":"dir-long"; const entry=Number(p.entry??p.entry_price??p.average_entry_price??p.price??0); const current=Number(p.last_price??p.current_price??p.mark_price??p.price??entry); const sl=Number(p.stop_loss??p.sl??0); const tpLevels=activeOrderTakeProfits(p); const tp1=tpLevels[0]??0; const tpPending=!p.pending_order&&sl>0&&!tp1; const tpLabel=tpPending?"TP PENDING":(tp1?fmtPrice(tp1):"-"); const trailing=Number(p.trailing_stop_loss??0); const trailingActive=!!p.trailing_active; const trailingStatus=String(p.trailing_status??(trailingActive?"active":"inactive")); const trailingLabel=trailingStatus==="active_updated"?"UPDATED":trailingStatus==="active_not_tightening"?"ACTIVE · WAIT":trailingStatus==="active_update_failed"?"ERROR":trailingActive?"ACTIVE":"INACTIVE"; const size=Number(p.remaining_size??p.quantity??p.qty??p.size??0); const modal=p.pending_order?0:positionMargin(p,entry,size); const pnl=p.pending_order?0:Number(p.unrealized_pnl??((isShort?(entry-current):(current-entry))*size)); const pnlCls=pnl>=0?"pnl-pos":"pnl-neg"; const pnlPct=modal>0?((pnl/modal)*100):0; const distToSl=sl&&entry?Math.abs(((sl-entry)/entry)*100):0; const distToTp=tp1&&entry?Math.abs(((tp1-entry)/entry)*100):0; const slWarn=sl&&current&&(isShort?(current>=sl*0.995):(current<=sl*1.005))?" sl-near":""; const tpWarn=tp1&&current&&(isShort?(current<=tp1*1.005):(current>=tp1*0.995))?" tp-near":""; const levRaw=Number(p.leverage??p.configured_leverage??0); const levBadge=Number.isFinite(levRaw)&&levRaw>=1?`<span class="mc-lev">${levRaw%1===0?levRaw:levRaw.toFixed(1)}x</span>`:""; const tr=`<tr data-symbol="${esc(sym)}" class="${dirCls}">
...
<td data-label="Stop Loss"><span id="ao-sl-${esc(sym)}" class="ao-sl${slWarn}">${sl?fmtPrice(sl):"-"}${sl&&distToSl?`<small>SL ${distToSl.toFixed(2)}%</small>`:""}</span></td>
 <td data-label="Take Profit"><span class="ao-tp${tpWarn}">${tpLabel}${tp1&&distToTp?`<small>${distToTp.toFixed(2)}% · RR ${Number(p.actual_risk_reward??0).toFixed(2)} · ${Number(p.take_profit_order_count??tpLevels.length??0)} order</small>`:""}</span></td>
 <td data-label="Trailing"><span id="ao-trailing-${esc(sym)}" class="ao-trail ${trailingActive?"trail-active":""}">${trailing?fmtPrice(trailing):"-"}${trailingActive?" ●":""}<small>${esc(trailingLabel)}${p.trailing_percent?` · ${Number(p.trailing_percent).toFixed(2)}%`:""}${p.trailing_candidate_stop?` · next ${fmtPrice(p.trailing_candidate_stop)}`:""}</small></span></td>
