...
function renderAgentEntries(items){
  ...
  const rows = items.map(item => {
    ...
    const conflictReason = conflictRejected
      ...
      : "";
    const reason = esc(decisionReason || conflictReason || eligibility);
    const action = esc(decision.action || (conflictRejected ? "REJECT" : (res.eligible ? "-" : "SKIP")));
