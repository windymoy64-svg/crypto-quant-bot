...
function orderHistory(orders){ const rows=list(orders?.closed_positions).length?[...list(orders.closed_positions),...list(orders?.order_history).filter(o=>o?.close_scope==="partial"||String(o?.bot_role||"").startsWith("take_profit_"))]:list(orders?.order_history).filter(o=>o?.close_scope==="partial"||String(o?.bot_role||"").startsWith("take_profit_")); return rows.map(o=>({...o,reason:o.reason??o.close_label??o.close_reason,reason_source:o.reason_source??"exchange_lifecycle"})); }
