...
function orderHistory(orders){ const rows=list(orders?.closed_positions).length?list(orders.closed_positions):list(orders?.order_history).filter(o=>String(o?.status||"").toUpperCase()==="CLOSED"); return rows.map(o=>({...o,reason:o.reason??o.close_label??o.close_reason})); }
...
function syncDashboardPanels(payload){
  ...
  state.chartExchange=String(multi.active_execution_exchange||state.chartExchange||"binance").toLowerCase();
...
function setChartSymbol(symbol){ if(!symbol) return; state.chartSymbol=symbol; const sel=byId("chart-symbol"); if(sel) sel.value=symbol; showView("scanner"); ensureTradingViewChart(); loadKlines().catch(err=>showToast(`chart: ${err.message}`)); }
...
function showView(view){ state.currentView=view||"overview"; document.querySelectorAll("[data-view-section]").forEach(s=>s.classList.toggle("active",s.dataset.viewSection===state.currentView)); document.querySelectorAll("[data-view-link]").forEach(l=>l.classList.toggle("active",l.dataset.viewLink===state.currentView)); const isMenuView=["portfolio","health","agents","settings"].includes(state.currentView); document.querySelector(".nav-menu-btn")?.classList.toggle("active",isMenuView); toggleMobileMenu(false); document.body.dataset.view=state.currentView; history.replaceState(null,"",`#${state.currentView}`); if(state.currentView==="scanner"){ ensureTradingViewChart(); loadKlines().catch(()=>{}); } setTimeout(resizeCharts,80); if(state.currentView==="agents") loadAgentPanels().catch(console.warn); }
