...
const state = { liveEvents:[], lastOrders:null, loading:true, multiPortfolio:null, tvChart:null, tvSeries:null, tvVolumeSeries:null, apexChart:null, renderEventsTimer:null, toastTimer:null, chartSymbol:"BTC/USDT", chartTimeframe:"1h", chartLimit:200, chartLoading:false, chartRefreshTimer:null, dashboardRefreshTimer:null, clockTimer:null, ws:null, wsReconnectTimer:null, wsSnapshotTimer:null, symbols:[...FALLBACK_SYMBOLS], configuredSymbols:[], currentView:"overview", liveEntryCandidates:[], lastEntryLiveTs:0, lastAgentPipeline:null, lastPayload:clone(DEFAULT_PAYLOAD) };
...
function orderHistory(orders){ return list(orders?.closed_positions).length?list(orders.closed_positions):list(orders?.order_history).filter(o=>String(o?.status||"").toUpperCase()==="CLOSED"); }
...
function liveExchangePositions(){
  ...
  return Number(multi?.accounts_connected??0)>0 ? list(multi.positions) : [];
}
