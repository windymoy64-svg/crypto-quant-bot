...
class ScoreEngine:
    def __init__(self, rules: list[dict[str, Any]], dynamic_weight_engine: DynamicWeightEngine | None = None) -> None:
        self.rules = rules
    ...
    def from_json(cls, path: str, weights_path: str | None = None) -> "ScoreEngine":
        ...
        return cls(data["rules"], dynamic_weight_engine=dynamic_weight_engine)
    ...
    def score(self, candles: list[Candle], market_regime: MarketRegime | str | None = None) -> ScoreResult:
        ...
        action = "BUY" if confidence >= 90 else "WATCH" if confidence >= 80 else "SKIP"
        return ScoreResult(
